...
class APIClient:
    ...
    async def login(
        self, telegram_id: int, username: str, first_name: str, language_code: str = "en"
    ) -> dict[str, Any]:
        """
    ...
    async def create_task(self, title: str, deadline: str | None = None, category_id: str | None = None) -> dict:
    ...
    async def update_task(self, task_id: str, data: dict):
        ...
        async with self.session.patch(
            f"{self.base_url}/tasks/{task_id}/", json=data, headers=self._get_headers()
        ) as resp:
            resp.raise_for_status()
            return await resp.json()
    ...
    async def delete_category(self, cat_id: str):
        """Delete category"""
        async with self.session.delete(f"{self.base_url}/categories/{cat_id}/", headers=self._get_headers()) as resp:
            resp.raise_for_status()
